...
- Pacote `quantilica-web` (closed-source) extraindo a infraestrutura comum dos 4 apps Flask de metadados (`aplicação privada`, `aplicação privada`, `aplicação privada`, `aplicação privada`).
